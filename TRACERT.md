...
Утилита TRACERT выполняет трассировку до точки назначения с помощью посылки адресату эхо-сообщений. Посылка осуществляется по протоколу Control Message Protocol (ICMP) с постоянным увеличением значений срока жизни пакетов (Time to Live, TTL).
...
Основания для проведения разработки является лабораторная работа №2 по дисциплине ТРПО. Лабораторная работа утверждена Сувальским А. А., в дальнейшем именуемым Заказчиком. Дата утверждения – 09.09.20.
...
Функциональным назначением утилиты является определение маршрута к месту назначения с помощью отправки эхо-сообщений протокола ICMP (Internet Control) пакетов в место назначения.

## <a name = "3.2" > 3.2. Эксплуатационное назначение </a>

---

Утилита TRACERT должна эксплуатироваться в интерпретаторе командной строки CMD или в пакетном файле.
...
1. функция вывода упорядоченного списка использованных маршрутизаторов;
2. функция отслеживания неполадок в сети.

## <a name = "4.1.2" > 4.1.2. Требования к организации входных и выходных данных </a>

---

4.1.2.1. Утилита TRACERT должна иметь синтаксис, изображенный на рис.1.

![Общий синтаксис команды TRACERT](k1.png)
Рис. 1 – Общий синтаксис команды TRACERT

Здесь используются:

- _-d_ – без разрешения в имена узлов;
- _-h_ максЧисло – максимальное число прыжков при поиске узла;
- _-j_ списокУзлов – свободный выбор маршрута по списку узлов (только IPv4);
- _-w_ таймаут – таймаут каждого ответа в миллисекундах;
- _-R_ – трассировка пути (только IPv6);
- _-S_ адресИсточника – используемый адрес источника (только IPv6);
- _-4_ – принудительное использование IPv4;
- _-6_ – принудительное использование IPv6.
...
В случае неправильного ввода команды TRACERT, командная строка должна вывести сообщение, данный случай описан на рис. 3.

![Неправильный ввод команды TRACERT](k2.png)
Рис. 2 – Неправильный ввод команды TRACERT
